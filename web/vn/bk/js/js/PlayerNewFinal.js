...
$(function () {
    ...
    //var menuid = action.substring(action.indexOf("(") + 1, action.indexOf(","));alert(menuid);
	var path = "";
	//alert(window.location.pathname);
	if ( window.location.pathname.indexOf("mediaplayOne")>=1)
	{
		//request.getParameter("isPlayOne")
		path = "/web1/media/vn/funring/audios/Xa Mot Cuoc Tinh - Anh Khang.mp3";
	}
	else
		path = '/web1/media/vn/funring/include/play_list.xml';
    ...
    var attributes = {};
    swfobject.embedSWF("/web/media/vn/funring/PlayerClassicAds.swf", "imuzikPlayerSwf", "1", "1", "9.0.124", 'false', flashvars, params, attributes, callbackFn);
    ...
    $("#play-button").click(function () {
        if (checkStatusPlaying() == true) {alert('1');
...
function onImuzikChangeSong(obj) {
    ...
    /*if (checkStatusIsRepeat() == false) {
        ...
        if (obj.index == minCanPlay && endOfRepeat == 0) {alert('n2');
            eventStopSongImuzik();
...
function playOneSongAllowed(playIndex) {alert(playIndex);
    var listsSong = player.getPlaylist();
    var maxLength = listsSong.length;
    var canPlay = 0;
...
function setTitle(obj) {
    ...
    setChangePositionController(0);//alert(title);
    //$("#title-marquee").text(title);
	$("#title-marquee").html(title);
    position = 50;
    songTitleInterval = setInterval('marqueeSongTitle();', 30);
