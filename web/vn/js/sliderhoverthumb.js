...
(function(factory){
	...
	$.cxSlide = function(){
		...
		slide.build = function(){
			...
			_this.dom.numList = _this.dom.el.find('.bsht-btn');
			...
			if (_this.settings.btn && !_this.dom.numList.length) {
				...
				_this.dom.numList = $('<ul></ul>',{'class': 'bsht-btn', 'html': _html}).appendTo(_this.dom.el);
				_this.dom.numBtns = _this.dom.numList.find('li');
			};
...
jQuery(document).ready(function($){
	$(".banner-slider-hover-container #slide_fade").cxSlide({
		events: "mouseover",
		type: "fade",
		speed: 300
	});
